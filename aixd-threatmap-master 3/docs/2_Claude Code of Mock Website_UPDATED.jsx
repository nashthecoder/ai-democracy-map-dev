...
function useSvgPanZoom(minScale = 0.5, maxScale = 4) {
  ...
  const wasJustDragged = () => justDraggedRef.current;
  const zoomBy = (factor) => setPanZoom((v) => ({ ...v, scale: Math.min(maxScale, Math.max(minScale, v.scale * factor)) }));
  const reset = () => setPanZoom({ x: 0, y: 0, scale: 1 });

  return { panZoom, isDragging, onWheel, onMouseDown, onMouseMove, onMouseUp, wasJustDragged, zoomBy, reset };
}

/* ============================================================
   Map A — bipartite morph: harm mechanism tiers ↔ pro-democracy
   activity clusters. Clicking a box or link morphs both columns in
   place instead of opening a side panel (see handover for the exact
   click → view table).
   ============================================================ */
const HM2A_LX = 34, HM2A_LW = 274, HM2A_RX = 676, HM2A_RW = 274, HM2A_TOP = 54, HM2A_BOT = 392;
const HM2A_NOTE_BASE = "a link means an entry mentions both together; thicker links are mentioned more often. Click any box or link to unfold the detail behind it.";

function hm2BuildViewA(st) {
  if (st.type === "overview") {
    return {
      headL: "Harm mechanisms",
      headR: "Pro-democracy activities",
      left: HM2_TIERS.map((t) => ({ c: t, label: HM2_TIER_SHORT[t], code: t, n: HM2_HB.tierQ[t], color: TIER_COLORS[t], ink: "#fff", act: "tier", t })),
      right: HM2_BENS.map((b) => ({ c: b, label: HM2_BEN_NAME[b], code: b, n: HM2_HB.benQ[b], color: HM2_BEN_COLOR[b], ink: HM2_LIGHT_BEN[b] ? "#22201a" : "#fff", act: "ben", b })),
      links: HM2_HB.edges.filter((e) => e.n >= 4).map((e) => ({ l: e.t, r: e.b, n: e.n, color: TIER_COLORS[e.t], act: "edge", t: e.t, b: e.b })),
      note: HM2A_NOTE_BASE,
    };
  }
  let rows, headL, headR, note;
  if (st.type === "tier") {
    rows = (HM2_HB.tier[st.t] || []).map((r) => [r[0], r[1], r[2]]);
  ...
  let min = 2, use = rows.filter((r) => r[2] >= min);
  if (use.length > 20) { min = 3; use = rows.filter((r) => r[2] >= min); }
  if (!use.length) { min = 1; use = rows; }
  const hidden = rows.length - use.length;
  const lTot = {}, rTot = {};
  use.forEach(([a, b, n]) => { lTot[a] = (lTot[a] || 0) + n; rTot[b] = (rTot[b] || 0) + n; });
  const left = Object.keys(lTot).sort((a, b) => hm2Cmp(hm2HarmKey(a), hm2HarmKey(b))).map((c) => ({
    c, label: hm2Lbl(c), code: c, n: lTot[c], color: TIER_COLORS[tierOf(c)] || TIER_COLORS[c], ink: "#fff",
    act: st.type === "ben" ? "edge" : null, t: c, b: st.b,
  }));
  const right = Object.keys(rTot).sort((a, b) => hm2Cmp(hm2BenKey(a), hm2BenKey(b))).map((c) => ({
    c, label: hm2Lbl(c), code: c, n: rTot[c], color: HM2_BEN_COLOR[hm2BenOf(c)],
    ink: HM2_LIGHT_BEN[hm2BenOf(c)] ? "#22201a" : "#fff",
    act: st.type === "tier" ? "edge" : null, t: st.t, b: c,
  }));
...
function BipartiteMorphMap({ onNoteChange }) {
  const [viewA, setViewA] = useState({ type: "overview" });
  const pz = useSvgPanZoom(0.5, 4);
  const width = 1040, height = 400;

  const v = useMemo(() => hm2BuildViewA(viewA), [viewA]);

  useEffect(() => {
    if (onNoteChange) onNoteChange((viewA.type === "overview" ? "" : "• ") + v.note);
  ...
  const svgHtml = useMemo(() => {
    const lY = {}, rY = {};
    const lH = (HM2A_BOT - HM2A_TOP) / Math.max(v.left.length, 1);
    const rH = (HM2A_BOT - HM2A_TOP) / Math.max(v.right.length, 1);
    v.left.forEach((d, i) => (lY[d.c] = HM2A_TOP + lH * i + lH / 2));
    v.right.forEach((d, i) => (rY[d.c] = HM2A_TOP + rH * i + rH / 2));
    const lBox = Math.min(34, lH - 5), rBox = Math.min(34, rH - 5);
    let s = "";
    s += `<text x="${HM2A_LX + HM2A_LW / 2}" y="30" text-anchor="middle" font-size="12.5" font-weight="800" fill="#000">${hm2Esc(v.headL)}</text>`;
    s += `<text x="${HM2A_RX + HM2A_RW / 2}" y="30" text-anchor="middle" font-size="12.5" font-weight="800" fill="#000">${hm2Esc(v.headR)}</text>`;
    s += `<line x1="${HM2A_LX}" y1="40" x2="${HM2A_LX + HM2A_LW}" y2="40" stroke="#D6D6CA" stroke-width="1"/>`;
    s += `<line x1="${HM2A_RX}" y1="40" x2="${HM2A_RX + HM2A_RW}" y2="40" stroke="#D6D6CA" stroke-width="1"/>`;
    v.links.forEach((k) => {
      const y1 = lY[k.l], y2 = rY[k.r];
      if (y1 == null || y2 == null) return;
      const x1 = HM2A_LX + HM2A_LW, x2 = HM2A_RX, mx = (x1 + x2) / 2, my = (y1 + y2) / 2;
      const attrs = k.act ? `data-act="${k.act}" data-t="${k.t}" data-b="${k.b}"` : "";
      s += `<g style="${k.act ? "cursor:pointer" : ""}" ${attrs}>
        <path d="M${x1} ${y1} C ${x1 + 80} ${y1}, ${x2 - 80} ${y2}, ${x2} ${y2}" fill="none"
          stroke="${k.color}" stroke-width="${(0.7 + k.n * 0.3).toFixed(1)}" opacity="0.4"/>
        <text x="${mx}" y="${my - 4}" text-anchor="middle" font-size="9" font-weight="700" fill="#22201a"
          stroke="#ffffff" stroke-width="2.5" paint-order="stroke">${k.n}</text></g>`;
    });
    function box(d, x, w, y, h) {
      const fs = h >= 26 ? 10 : 9;
      const all = hm2WrapText(d.label, h >= 26 ? 26 : 30);
      const lines = all.slice(0, h >= 26 ? 2 : 1);
      const trunc = all.length > lines.length;
      const attrs = d.act ? `data-act="${d.act}" data-t="${d.t || d.c}" data-b="${d.b || d.c}"` : "";
      let g = `<g style="${d.act ? "cursor:pointer" : ""}" ${attrs}>
        <rect x="${x}" y="${(y - h / 2).toFixed(1)}" width="${w}" height="${h.toFixed(1)}" rx="9" fill="${d.color}" opacity="0.88" stroke="#fff" stroke-width="1.5"/>
        <text x="${x + 9}" y="${y}" dominant-baseline="central" font-size="${fs}" font-weight="800" fill="${d.ink}">${hm2Esc(d.code)}</text>`;
      const tx = x + 9 + d.code.length * 5.6 + 8;
      if (lines.length === 1) g += `<text x="${tx}" y="${y}" dominant-baseline="central" font-size="${fs - 0.5}" font-weight="600" fill="${d.ink}">${hm2Esc(lines[0])}${trunc ? "…" : ""}</text>`;
      else g += `<text x="${tx}" y="${y - 5}" dominant-baseline="central" font-size="${fs - 0.5}" font-weight="600" fill="${d.ink}">${hm2Esc(lines[0])}</text>
          <text x="${tx}" y="${y + 5.5}" dominant-baseline="central" font-size="${fs - 0.5}" font-weight="600" fill="${d.ink}">${hm2Esc(lines[1])}${trunc ? "…" : ""}</text>`;
      g += `<text x="${x + w - 8}" y="${y}" text-anchor="end" dominant-baseline="central" font-size="${fs}" font-weight="700" fill="${d.ink}">${d.n}</text></g>`;
      return g;
    }
    v.left.forEach((d) => (s += box(d, HM2A_LX, HM2A_LW, lY[d.c], lBox)));
    v.right.forEach((d) => (s += box(d, HM2A_RX, HM2A_RW, rY[d.c], rBox)));
    return s;
  }, [v]);

  const legendItems = useMemo(() => {
    const tiersShown = [...new Set(v.left.map((d) => tierOf(d.c) || d.c))].sort();
    const bensShown = [...new Set(v.right.map((d) => hm2BenOf(d.c)))].sort((a, b) => hm2Cmp(hm2BenKey(a), hm2BenKey(b)));
    const codesL = {}, codesR = {};
    v.left.forEach((d) => { const k = tierOf(d.c) || d.c; (codesL[k] = codesL[k] || []).push(d.c); });
    v.right.forEach((d) => { const k = hm2BenOf(d.c); (codesR[k] = codesR[k] || []).push(d.c); });
    const items = [];
  ...
  const handleSvgClick = (e) => {
    ...
    if (act === "tier") changeView({ type: "tier", t: el.getAttribute("data-t") });
    else if (act === "ben") changeView({ type: "ben", b: el.getAttribute("data-b") });
    else if (act === "edge") changeView({ type: "edge", t: el.getAttribute("data-t"), b: el.getAttribute("data-b") });
  };
    ...
    <div>
      <div style={{ position: "relative" }}>
        <svg
          ...
          style={{ display: "block", cursor: pz.isDragging ? "grabbing" : "grab", touchAction: "none" }}
          onWheel={pz.onWheel} onMouseDown={pz.onMouseDown} onMouseMove={pz.onMouseMove} onMouseUp={pz.onMouseUp} onMouseLeave={pz.onMouseUp}
          onClick={handleSvgClick}
        >
          <g transform={`translate(${pz.panZoom.x}, ${pz.panZoom.y}) scale(${pz.panZoom.scale})`} dangerouslySetInnerHTML={{ __html: svgHtml }} />
        </svg>
        {viewA.type !== "overview" && (
          <button
            onClick={() => changeView({ type: "overview" })}
            style={{
              position: "absolute", left: 10, top: 8, zIndex: 5, borderRadius: 999,
              border: `1px solid ${BRAND.border}`, background: "#fff", color: "#333", fontFamily: FONT_BODY,
        ...
        <ZoomControls onZoomIn={() => pz.zoomBy(1.25)} onZoomOut={() => pz.zoomBy(0.8)} onReset={pz.reset} />
      </div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: "14px 20px", marginTop: 14, paddingTop: 12, borderTop: `1px solid ${BRAND.border}` }}>
        {legendItems.map((item) => (
          <div key={item.key} style={{ fontSize: 10.5, color: BRAND.muted, maxWidth: 190, lineHeight: 1.5 }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 5, fontWeight: 700, color: "#333", marginBottom: 2 }}>
...
/* ============================================================
   Map B — clustered pathway bands: recurring causal chains from
   conditions of AI development through to specific democracy
   aspects affected. Geometry below is the colleague's original
   430px-tall layout uniformly rescaled to this site's 400px map
   height (scale factor 400/430 ≈ 0.930) so all carousel panels
   stay the same size.
   ============================================================ */
const HM2B_TX = { T0: 70, T1: 182, T2: 288, T3: 396, T4: 500, T5: 604, T6: 698, T7: 782 };
const HM2B_CLUS_X = [[14, 126], [126, 342], [342, 656], [656, 838]];
const HM2B_DAX = 848, HM2B_DAW = 182;
const HM2B_BAND = [{ top: 69, h: 136 }, { top: 214, h: 93 }, { top: 318, h: 78 }];
const HM2B_NODE_W = 98;
const HM2B_TITLE_Y1 = 15, HM2B_TITLE_Y2 = 26, HM2B_CHIP_Y = 43;
const HM2B_VLINE_BOTTOM = 385;

function hm2TierOfPath(c) {
  const m = /^T(\d+)/.exec(c);
  return m ? "T" + m[1] : null;
}

function hm2RenderBHtml() {
  let s = "";
  HM2B_CLUS_X.forEach((cx, ci) => {
    const c = HM2_CLUSTERS[ci], mid = (cx[0] + cx[1]) / 2;
    if (ci > 0) s += `<line x1="${cx[0] - 4}" y1="6" x2="${cx[0] - 4}" y2="${HM2B_VLINE_BOTTOM}" stroke="#D6D6CA" stroke-width="1"/>`;
    const lines = hm2WrapText(c.name, 20).slice(0, 2);
    if (lines.length === 1) s += `<text x="${mid}" y="${HM2B_TITLE_Y2}" text-anchor="middle" font-size="11.5" font-weight="800" fill="#000">${hm2Esc(lines[0])}</text>`;
    else s += `<text x="${mid}" y="${HM2B_TITLE_Y1}" text-anchor="middle" font-size="11.5" font-weight="800" fill="#000">${hm2Esc(lines[0])}</text>
        <text x="${mid}" y="${HM2B_TITLE_Y2}" text-anchor="middle" font-size="11.5" font-weight="800" fill="#000">${hm2Esc(lines[1])}</text>`;
    c.tiers.forEach((t) => {
      s += `<text x="${HM2B_TX[t]}" y="${HM2B_CHIP_Y}" text-anchor="middle" font-size="8.5" font-weight="700" fill="${TIER_COLORS[t]}">${hm2Esc(HM2_TIER_SHORT[t])}</text>`;
    });
  });
  s += `<line x1="${HM2B_DAX - 8}" y1="6" x2="${HM2B_DAX - 8}" y2="${HM2B_VLINE_BOTTOM}" stroke="#D6D6CA" stroke-width="1"/>`;
  s += `<text x="${HM2B_DAX + HM2B_DAW / 2}" y="${HM2B_TITLE_Y1}" text-anchor="middle" font-size="11.5" font-weight="800" fill="#000">Democracy</text>
      <text x="${HM2B_DAX + HM2B_DAW / 2}" y="${HM2B_TITLE_Y2}" text-anchor="middle" font-size="11.5" font-weight="800" fill="#000">aspects affected</text>`;

  HM2_PATHS.forEach((f, fi) => {
    const b = HM2B_BAND[fi];
    s += `<rect x="10" y="${b.top - 16}" width="1020" height="${b.h}" rx="14" fill="${fi % 2 ? "#FBFBF3" : "#F7F7EE"}" stroke="#E4E4D6" stroke-width="1"/>`;
    s += `<rect x="10" y="${b.top - 16}" width="5" height="${b.h}" rx="2.5" fill="#963735" opacity="0.75"/>`;
    s += `<text x="24" y="${b.top - 4}" font-size="11.5" font-weight="800" fill="#000">${hm2Esc(f.name)}</text>`;
    s += `<text x="${24 + f.name.length * 6.4}" y="${b.top - 4}" font-size="9.5" font-weight="600" fill="#9A9A92"> · ${f.units} entries</text>`;
    const lanes = Math.max(...f.nodes.map((n) => n.l)) + 1;
    const laneH = (b.h - 26) / lanes;
    const nx = {}, ny = {};
    f.nodes.forEach((n) => { nx[n.c] = HM2B_TX["T" + n.t]; ny[n.c] = b.top + 2 + n.l * laneH + laneH / 2; });
    f.segs.forEach((sg, si) => {
      const x1 = nx[sg.a] + HM2B_NODE_W / 2, y1 = ny[sg.a], x2 = nx[sg.b] - HM2B_NODE_W / 2, y2 = ny[sg.b];
      const mx = (x1 + x2) / 2, my = (y1 + y2) / 2, col = TIER_COLORS[hm2TierOfPath(sg.b)];
      s += `<g style="cursor:pointer" data-act="seg" data-fi="${fi}" data-si="${si}">
        <path d="M${x1} ${y1} C ${x1 + 24} ${y1}, ${x2 - 24} ${y2}, ${x2} ${y2}" fill="none" stroke="${col}" stroke-width="${(0.9 + sg.n * 0.42).toFixed(1)}" opacity="0.55"/>
        <path d="M${x2 - 6} ${y2 - 3.5} L${x2} ${y2} L${x2 - 6} ${y2 + 3.5}" fill="none" stroke="${col}" stroke-width="1.3" opacity="0.8"/>
        <text x="${mx}" y="${my - 4}" text-anchor="middle" font-size="8.5" font-weight="700" fill="#22201a" stroke="#ffffff" stroke-width="2.5" paint-order="stroke">${sg.n}</text></g>`;
    });
    f.nodes.forEach((n) => {
      const x = nx[n.c], y = ny[n.c], col = TIER_COLORS["T" + n.t];
      const h = Math.min(30, laneH - 5);
      const all = hm2WrapText(hm2Lbl(n.c), 15), lines = all.slice(0, 2), trunc = all.length > 2;
      s += `<g style="cursor:pointer" data-act="node" data-c="${n.c}">
        <rect x="${x - HM2B_NODE_W / 2}" y="${y - h / 2}" width="${HM2B_NODE_W}" height="${h}" rx="8" fill="${col}" opacity="0.88" stroke="#fff" stroke-width="1.5"/>`;
      if (lines.length === 1) s += `<text x="${x}" y="${y}" text-anchor="middle" dominant-baseline="central" font-size="8.5" font-weight="700" fill="#fff">${hm2Esc(lines[0])}</text>`;
      else s += `<text x="${x}" y="${y - 4.5}" text-anchor="middle" dominant-baseline="central" font-size="8.5" font-weight="700" fill="#fff">${hm2Esc(lines[0])}</text>
            <text x="${x}" y="${y + 5.5}" text-anchor="middle" dominant-baseline="central" font-size="8.5" font-weight="700" fill="#fff">${hm2Esc(lines[1])}${trunc ? "…" : ""}</text>`;
      s += `</g>`;
    });
    const das = f.das.slice(0, 4);
    das.forEach((d, di) => {
      const step = (b.h - 26) / das.length, y = b.top + 2 + di * step + step / 2;
      const hh = Math.min(20, step - 3), col = HM2_DA_PILLAR[d[0][0]];
      s += `<g style="cursor:pointer" data-act="da" data-fi="${fi}" data-di="${di}">
        <rect x="${HM2B_DAX}" y="${y - hh / 2}" width="${HM2B_DAW}" height="${hh}" rx="${hh / 2}" fill="${col}" opacity="0.88" stroke="#fff" stroke-width="1.2"/>
        <text x="${HM2B_DAX + 9}" y="${y}" dominant-baseline="central" font-size="8.5" font-weight="800" fill="#fff">${d[0]}</text>
        <text x="${HM2B_DAX + 31}" y="${y}" dominant-baseline="central" font-size="8" font-weight="600" fill="#fff">${hm2Esc(hm2WrapText(HM2_DA_NAME[d[0]], 27)[0])}${hm2WrapText(HM2_DA_NAME[d[0]], 27).length > 1 ? "…" : ""}</text>
        <text x="${HM2B_DAX + HM2B_DAW - 8}" y="${y}" text-anchor="end" dominant-baseline="central" font-size="8.5" font-weight="700" fill="#fff">${d[1]}</text></g>`;
    });
  });
  return s;
}

function PathwayBandsMap() {
  const [detail, setDetail] = useState("Click a mechanism, a step, or a democracy-aspect chip for detail.");
  const pz = useSvgPanZoom(0.5, 4);
  const width = 1040, height = 400;

  const svgHtml = useMemo(() => hm2RenderBHtml(), []);
  ...
  const handleSvgClick = (e) => {
    ...
    const act = el.getAttribute("data-act");
    if (act === "seg") {
      const fi = +el.getAttribute("data-fi"), si = +el.getAttribute("data-si");
      const f = HM2_PATHS[fi], sg = f.segs[si];
      setDetail(`<b>${hm2Esc(hm2Lbl(sg.a))} → ${hm2Esc(hm2Lbl(sg.b))}</b> — asserted together in <b>${sg.n} entries</b>. Pathway: ${hm2Esc(f.name)}.`);
    } else if (act === "node") {
      const c = el.getAttribute("data-c");
      const t = hm2TierOfPath(c);
      const clusterName = HM2_CLUSTERS.find((k) => k.tiers.includes(t))?.name || "";
      setDetail(`<b>${hm2Esc(hm2Lbl(c))} (${c})</b> — ${hm2Esc(HM2_TIER_SHORT[t])}, in ${hm2Esc(clusterName)}.`);
    } else if (act === "da") {
      const fi = +el.getAttribute("data-fi"), di = +el.getAttribute("data-di");
      const f = HM2_PATHS[fi], d = f.das[di];
      setDetail(`<b>${d[0]} ${hm2Esc(HM2_DA_NAME[d[0]])}</b> — mentioned in <b>${d[1]} of ${f.units} entries</b> in the pathway "${hm2Esc(f.name)}".`);
    }
  };
  ...
  return (
    <div>
      ...
      <div
        style={{ marginTop: 14, padding: "12px 14px", background: "#EAEAE3", borderRadius: 14, fontSize: 12.5, color: BRAND.muted, lineHeight: 1.55 }}
        dangerouslySetInnerHTML={{ __html: detail }}
      />
      <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "8px 18px", fontSize: 10.5, color: BRAND.muted, marginTop: 14, paddingTop: 12, borderTop: `1px solid ${BRAND.border}` }}>
        <span style={{ fontWeight: 800, color: "#000", textTransform: "uppercase", letterSpacing: "0.06em" }}>Democracy aspects</span>
        {["1", "2", "3", "4"].map((p) => (
          <span key={p} style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
            <span style={{ width: 8, height: 8, borderRadius: 999, background: HM2_DA_PILLAR[p] }} />
...
export default function App() {
  ...
  const [mapANote, setMapANote] = useState(HM2A_NOTE_BASE);

  const panels = [
    {
      title: "Which democracy aspects are more frequent across threats?",
      highlights: ["democracy aspects"],
      note: "* here we can add something like the data collected does not include information on which tactics/threats are more effective or grave",
    ...
    {
      title: "Which pro-democracy activities are frequently linked to which harm mechanisms?",
      highlights: ["pro-democracy activities", "harm mechanisms"],
      note: mapANote,
      render: () => <BipartiteMorphMap onNoteChange={setMapANote} />,
    },
    {
      title: "Through which pathways does the literature link AI to harm for democracy?",
      highlights: ["pathways", "harm for democracy"],
      note: "* each band is a recurring pathway; numbers count entries asserting that step within a single statement. Pathways are claims recurring in the literature, not verified causation.",
      render: () => <PathwayBandsMap />,
    },
  ];
